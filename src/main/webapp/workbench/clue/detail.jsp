...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#activityBodyBySearchName").on("click",$("input[name=choice]"),function(){
			...
			if(event.keyCode == 13){
				...
				$.ajax({
					...
					success : function(data){
						...
						$("#activityBodyBySearchName").html(html);
					}
				})


				//模态窗口里面默认的回车行为会刷新整个页面,这里需要在展现完列表之后,不让这个行为发生
				return false;
			}
		})

		$("#associationBtn").click(function(){
			var $choices = $("input[name=choice]:checked");
			if($choices.length == 0){
				alert("请选择要关联的市场活动");
			}
			else{
				var parm = "clueId=${c.id}";
				$.each($choices,function(i,n){
					parm += "&activityId=" + n.value;
				})
				$.ajax({
		...
		$.ajax({
			...
			success : function(data){
				...
				$.each(data,function(i,n){
					...
					html += '<td>'+n.endDate+'</td>';
					html += '<td>'+n.owner+'</td>';
					html += '<td><a href="javascript:void(0);" style="text-decoration: none;" onclick="Disassociation(\''+n.id+'\')"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>';
					html += '</tr>';
				})
	...
	function Disassociation(id){
		$.ajax({
			url : "workbench/clue/Disassociation.do",
			data : {
				"id" : id   //这里查表的时候把市场活动的id改成了线索市场活动表的id,因此直接传
			},
			type : "post",
			dataType : "json",
			success : function(data){
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<table id="activityTable" class="table table-hover" style="width: 900px; position: relative;top: 10px;">
						...
						<tbody id="activityBodyBySearchName">
<%--							<tr>
