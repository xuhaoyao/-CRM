<%@ page import="com.scnu.crm.settings.domain.DicValue" %>
<%@ page import="java.util.List" %>
<%@ page import="java.util.Map" %>
<%@ page import="java.util.Set" %>
<%@ page import="com.scnu.crm.workbench.domain.Tran" %>
...
<script type="text/javascript">
	...
	function changeStage(stage,i){  //stage:需要改变的阶段 i:对应下标
		if(stage == $("#stage").val()) return false;
		$.ajax({
	...
	function changeIcon(stage,i){
		var index = "<%=index%>";    //正常阶段和后面丢失阶段的分界点坐标
		var len = "<%=dvList.size()%>";
		var currentPossibility = $("#possibility").html();
		if(currentPossibility == "0"){
			for(var j = 0 ;j < index; j++){
				//黑圈
				$("#" + j).removeClass();  //移除原有样式
			...
			for(var j = index; j < len; j++){
				if(j == i){
					//红叉
					$("#" + j).removeClass();  //移除原有样式
					$("#" + j).addClass("glyphicon glyphicon-remove mystage");  //添加新样式
					$("#" + j).css("color","#FF0000");
				}
				else{
					//黑叉
			...
			for(var j = 0; j < len; j++){
				if(j == i){
					//绿色标记
					$("#" + j).removeClass();  //移除原有样式
					$("#" + j).addClass("glyphicon glyphicon-map-marker mystage");  //添加新样式
					$("#" + j).css("color","#90F790");
				}else if(j < i){
					//绿圈
					$("#" + j).removeClass();  //移除原有样式
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			//如果当前阶段不为0 前面有绿圈 绿色标记,黑圈,后面两个是黑叉
			else{
				//找当前阶段的下标
				int currentIndex = 0;
				for(int i = 0; i < dvList.size(); i++){
...
<%--		<span class="glyphicon glyphicon-ok-circle mystage" data-toggle="popover" data-placement="bottom" data-content="资质审查" style="color: #90F790;"></span>
	...
	<div style="position: relative; top: 0px;">
		...
		<div style="position: relative; left: 40px; height: 30px; top: 40px;">
			...
			<div style="width: 300px;position: relative; left: 650px; top: -60px;"><b>${t.activityId eq null ? "未填写" : t.activityId}</b></div>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px;"></div>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px; left: 450px;"></div>
		</div>
		<div style="position: relative; left: 40px; height: 30px; top: 50px;">
			<div style="width: 300px; color: gray;">联系人名称</div>
			<div style="width: 500px;position: relative; left: 200px; top: -20px;"><b>${t.contactsId eq null ? "未填写" : t.contactsId}</b></div>
